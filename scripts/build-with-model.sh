...
PRIMARY_FILENAME="gemma-2-2b-q4-km.gguf"
FALLBACK_URL="https://huggingface.co/bartowski/gemma-2-2b-it-GGUF/resolve/main/gemma-2-2b-it-Q4_K_S.gguf"
FALLBACK_FILENAME="gemma-2-2b-q4-kos.gguf"
...
download_model "$PRIMARY_URL" "$MODEL_DIR/$PRIMARY_FILENAME" "Primary model (Q4_K_M)" 1740000000 || exit 1

# Download fallback model (unless --primary-only)
if [ "$PRIMARY_ONLY" = false ]; then
    download_model "$FALLBACK_URL" "$MODEL_DIR/$FALLBACK_FILENAME" "Fallback model (Q4_K_S)" 1580000000 || exit 1
fi
